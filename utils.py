...
def find_close_books(image_path, 
    ...
    #Query  
    list_ids_distances = newIndex.knnQueryBatch(query_image_descriptors, k = k_neighbors, num_threads = 4)
    ...
    list_of_dict_idx_and_inliers = sorted(inliers_counts, key=lambda k: k['inliers'], reverse=True)[:top_n]
    result = [(item["index"], item["inliers"]) for item in list_of_dict_idx_and_inliers]
    return result
